...
A positive integer $c$ and an undirected graph, such that there exists a simple path that includes exactly $c$ edges.
...
- $V_L$:
  ...
  - Check that there are exactly $c+1$ integers in $y$.
...
- $T_{L^{\prime}L}$:
  ...
  - Let $c = n^{\prime}-1$ in $T_{L^{\prime}L}(x^{\prime})$.
